...
public class RuleLockLogicFilter implements LogicFilter<RuleFilterResEntity.LotteryCenterEntity> {
    ...
    public RuleFilterResEntity<RuleFilterResEntity.LotteryCenterEntity> filter(RuleFilterReqEntity ruleFilterReqEntity) {
        log.info("规则过滤-次数锁过滤开始 userId:{} strategyId:{} ruleModel:{}", ruleFilterReqEntity.getUserId(), ruleFilterReqEntity.getStrategyId(), ruleFilterReqEntity.getRuleModel());
        // 1. 查询规则的值（解锁所需次数，如3）
        ...
        if (userLotteryCount < Long.parseLong(ruleValue)) {
            log.info("规则过滤-次数锁拦截 userId:{} strategyId:{} ruleModel:{} awardId: {}", ruleFilterReqEntity.getUserId(), ruleFilterReqEntity.getStrategyId(), ruleFilterReqEntity.getRuleModel(),101);
        ...
        // 大于等于该值，放行
        log.info("规则过滤-次数锁放行 userId:{} strategyId:{} ruleModel:{}", ruleFilterReqEntity.getUserId(), ruleFilterReqEntity.getStrategyId(), ruleFilterReqEntity.getRuleModel());
        return RuleFilterResEntity.<RuleFilterResEntity.LotteryCenterEntity>builder()
